...
function assessPerformance() {
  ...
  var experimentData = jsPsych.data
    .get()
    .filter({ exp_stage: 'test', trialID: 'test_trial' }).trials;
  var missedCount = 0;
  var trialCount = 0;
  var rtArray = [];
  var rt = 0;
  ...
  for (var i = 0; i < experimentData.length; i++) {
    if (experimentData[i].trialID == 'test_trial') {
      trialCount += 1;
  ...
  Object[1](choiceCounts).forEach(function(key, index) {
...
/* TASK TEXT */
/* ******************************* */
var feedbackText = '<p class = center-block-text>Starting practice. Press <i>enter</i> to begin.</p>';
var possibleResponses = [['index finger', ',', 'comma key (,)'],
...
var choices = [possibleResponses[0][1], possibleResponses[1][1]];
var practiceLen = 16 / 16; // must be divisible by 4
...
var instructionNode = {
  ...
  loop_function: function() {
    data = jsPsych.data.get().filter({ trialID: 'instructions' }).trials;
...
var practiceNode = {
  ...
  loop_function: function(data) {
    ...
    for (var i = 0; i < data.trials.length; i++) {
      if (data.trials[i].trialID == 'practice_trial') {
        totalTrials += 1;
...
var testNode = {
  ...
  loop_function: function(data) {
    ...
    for (var i = 0; i < data.trials.length; i++) {
      if (data.trials[i].trialID == 'test_trial') {
        totalTrials += 1;
        if (data.trials[i].rt != null) {
          sumRT += data.trials[i].rt;
          sumResponses += 1;
...
'cued_task_switching_rdoc' // for this experiment

...
colors.forEach(function (color, index) {
  var stimulus = stimulus_text.replace("XXX", color).replace("YYY", color); // so each color and word are equivalent, can just use color for both
  var data = {
    trial_id: "stim",
    condition: "congruent",
    stim_color: color,
    stim_word: words[index],
    correct_response: possible_responses[index][1],
  };
  ...
  congruent_stim.push({
    ...
    stimulus: stimulus,
    data: data,
    key_answer: possible_responses[index][1],
  });
});
...
colors.forEach(function (color, colorIndex) {
  words.forEach(function (word, wordIndex) {
    if (color !== word) {
      var stimulus = stimulus_text.replace("XXX", color).replace("YYY", word);
      var data = {
        trial_id: "stim",
        condition: "incongruent",
        stim_color: color,
        stim_word: word,
        correct_response: possible_responses[colorIndex][1],
      };

      incongruent_stim.push({
        type: jsPsychHtmlKeyboardResponse,
        stimulus: stimulus,
        data: data,
        key_answer: possible_responses[colorIndex][1],
      });
    }
  });
});

console.log(congruent_stim);
console.log(incongruent_stim);
...
var choices = [
  ...
  possible_responses[2][1],
];

var speed_reminder =
  "<p class = block-text>Try to respond as quickly and accurately as possible.</p>";
var response_keys =
  '<ul list-text><li><span class = "large" style = "color:red">WORD</span>: ' +

...
var trial_proportions = [
  ...
  "AY",
  "BY",
]; // repeats n = 4 times for test block
...
var prompt_text =
  ...
  possible_responses[0][0] +
  "</li>" +
  '<p class = center-block-text style = "font-size:16px; line-height:80%%;">Anything else: ' +
  possible_responses[1][0] +
  "</li>" +
  "</div>";
// speed reminder
...
var rest_block = {
  ...
  choices: ["ALL_KEYS"],
  stimulus:
    "<div class = centerbox><p class = block-text>Take a break! Press any key to continue.</p></div>",
  post_trial_gap: 1000,
};
...
for (i = 0; i < numPracticeTrials; i++) {
  ...
  var wait_block = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function () {
      var trial_index_last_practice = jsPsych.data
        .get()
        .filter({ exp_stage: "practice" }).trials.length;

      console.log(trial_index_last_practice);
      return get_trial_stim(trial_index_last_practice);
    },
    is_html: true,
    choices: ["NO_KEYS"],
    data: {
...
function get_trial_stim(trial_id) {
  var match_to_last = numPracticeTrials * 4 - 1; //last in practice run
  switch (trial_id) {
    case match_to_last:
      return "Final Practice Trial Over";
    default:
      return "Trial over, get ready for the next one.";
  }
}
...
for (i = 0; i < numTrialsPerBlock; i++) {
  ...
  var wait_block = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus:
      "<div class = centerbox><div class = AX_feedback>Trial over, get ready for the next one.</div></div>",
    is_html: true,
    choices: ["NO_KEYS"],
    data: {
      trial_id: "wait",

...
function assessPerformance() {
  ...
  for (var i = 0; i < experiment_data.length; i++) {
    // if (experiment_data[i].trial_id == "test_trial") { // dont need if already filtered
    trial_count += 1;
  ...
  credit_var = missed_percent < 0.25 && avg_rt > 200 && accuracy > 0.6;
...
var practice_stimuli = [
  {
    //To change go:nogo ratio, add or remove one or more sub-dictionaries within practice_stimuli and test_stimuli_block
    stimulus:
...
// for testing
// var exp_len = 14; //multiple of numTrialsPerBlock
// var numTrialsPerBlock = 7; // multiple of 7 (6go:1nogo)
// var numTestBlocks = exp_len / numTrialsPerBlock;
...
var practiceNode = {
  ...
  loop_function: function (data) {
    ...
    if (accuracy > accuracy_thresh || practiceCount == practice_thresh) {
      ...
      block_stims = jsPsych.randomization.repeat(
        test_stimuli_block,
        numTrialsPerBlock / test_stimuli_block.length
      );
      return false;
    } else {
      feedback_text =
      ...
      block_stims = jsPsych.randomization.repeat(
        practice_stimuli,
        practice_len / practice_stimuli.length
      );
      return true;
    }
  },
